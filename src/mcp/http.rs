...
use crate::features::chat_read_api::{
    ChatReadApi,
    catalog::{CatalogColumn as ManifestColumn, CatalogTable as ManifestTable, PublicCatalog},
};
...
// Public HTTP runtime is owned by the binary adapter.
pub async fn run_public_http() -> anyhow::Result<()> {
    ...
    let catalog = PublicCatalog::load(&manifest_path)?;
    let api = Arc::new(ChatReadApi::new(pool.clone(), catalog.scope(), catalog)?);
    api.validate().await?;
...
async fn validate_views(pool: &PgPool, manifest: &Manifest) -> anyhow::Result<()> {
    manifest.validate_views(pool).await
}
...
async fn call_tool(state: &AppState, name: &str, arguments: Value) -> Result<Value, String> {
    match name {
        ...
        "chat.get_message" => {
            ...
            chat_get_message(&state.pool, &state.manifest, state.api.scope(), args).await
        }
        "chat.get_user_profile" => {
...
async fn chat_get_message(
    pool: &PgPool,
    manifest: &Manifest,
    scope: crate::features::chat_read_api::types::ChatReadScope,
    args: ChatMessageArgs,
) -> Result<Value, String> {
    if args.chat_id != scope.discussion_chat_id {
...
mod tests {
    use super::*;
    use crate::features::chat_read_api::catalog::ensure_identifier;
    #[test]
