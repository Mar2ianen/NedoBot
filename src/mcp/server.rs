...
impl ChatMcpServer {
    ...
    #[tool(
        ...
        description = "Точно считает matching-сообщения публичного чата по тем же фильтрам, что и chat.search_messages. Используй для вопросов 'сколько сообщений', 'в скольких сообщениях' и явного количества написанных сообщений. Поле query можно опустить для structural count. Для точных типов используй has_photo, has_video, has_document, has_audio, has_voice, has_sticker или has_animation; has_media означает любое медиа. reply_to_message_id и include_forwards сохраняют соответствующий scope. Не используй как число событий, уникальных авторов или вхождений внутри сообщения."
    )]
