...
impl ChatMcpServer {
    ...
    #[tool(
        ...
        description = "Точно считает matching-сообщения публичного чата по тем же фильтрам, что и chat.search_messages. Используй для вопросов 'сколько сообщений', 'в скольких сообщениях' и явного количества написанных сообщений. Поле query можно опустить для подсчёта всех сообщений по структурным фильтрам. Не используй как число событий, уникальных авторов или вхождений внутри сообщения."
    )]
