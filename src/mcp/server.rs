...
impl ChatMcpServer {
    ...
    #[tool(
        ...
        description = "Точно считает matching-сообщения публичного чата по тем же фильтрам, что и chat.search_messages. Используй для вопросов 'сколько сообщений', 'в скольких сообщениях' и явного количества написанных сообщений. Поле query можно опустить для structural count. Для точных типов используй has_photo, has_video, has_document, has_audio, has_voice, has_sticker или has_animation; has_media означает любое медиа. reply_to_message_id выбирает ответы на конкретное сообщение, has_reply фильтрует, является ли само сообщение reply; он не означает, что на сообщение кто-то ответил. include_forwards добавляет все пересылки к обычным сообщениям, is_forwarded фильтрует любую пересылку, а is_automatic_forward — только автоматическую пересылку. В результатах is_forwarded=true означает, что автор строки не обязательно является автором исходного текста; forwarded_from показывает источник, если он известен. Не используй как число событий, уникальных авторов или вхождений внутри сообщения."
    )]
