...
async fn user_top_words(
    ...
    let rows = sqlx::query_as::<_, (String, i64)>(
        ...
        where m.chat_id = $1
          ...
          and word !~ '^[0-9]+$'
          and word <> all(array[
              'что', 'это', 'как', 'так', 'для', 'про', 'или', 'уже', 'все', 'ещё', 'еще',
              'там', 'тут', 'тоже', 'если', 'надо', 'можно', 'будет', 'был', 'была', 'были',
              'они', 'она', 'оно', 'его', 'ее', 'её', 'мне', 'тебе', 'тебя', 'себя', 'нас',
              'вам', 'вас', 'кто', 'где', 'чем', 'без', 'при', 'под', 'над', 'после', 'пока',
              'the', 'and', 'for', 'you', 'are', 'not', 'with', 'this', 'that'
          ])
        group by word
