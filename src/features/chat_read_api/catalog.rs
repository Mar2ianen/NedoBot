...
use super::policy::{DISCUSSION_CHAT_ID, SOURCE_CHANNEL_ID};
...
fn load_manifest(path: &str) -> anyhow::Result<Manifest> {
    ...
    if manifest.version != 1
        ...
        || manifest.scope.discussion_chat_id != DISCUSSION_CHAT_ID
        || manifest.scope.source_channel_id != SOURCE_CHANNEL_ID
...
async fn chat_get_message(
    ...
    if args.chat_id != DISCUSSION_CHAT_ID {
