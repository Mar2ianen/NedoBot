...
use teloxide::types::{
    InlineKeyboardButton, InlineKeyboardMarkup, InputRichBlock, InputRichBlockBlockQuotation,
    InputRichBlockParagraph, InputRichBlockSectionHeading, InputRichMessage, RichText,
...
pub fn render_report(card: &ReportCard) -> InputRichMessage {
    ...
    blocks.push(InputRichBlock::Blockquote(InputRichBlockBlockQuotation {
        blocks: vec![InputRichBlock::Paragraph(paragraph([plain(truncate(
            card.target_text
                .as_deref()
...
mod tests {
    ...
    fn renders_rich_blocks_and_moderation_keyboard() {
        ...
        assert!(
            blocks
                .iter()
                .any(|block| matches!(block, InputRichBlock::Blockquote(_)))
        );
