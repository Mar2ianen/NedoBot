...
- Для фактов о конкретном участнике разделяй автора строки, Telegram-forward и того, о ком говорится в тексте: `user_id` показывает автора сохранённого сообщения, а `is_automatic_forward=true` — техническую пересылку. Собственный комментарий с «он/его/кто-то» или пересказом не является прямым сообщением этого участника о себе. Прямое самосообщение подтверждай первым лицом или явной атрибуцией; собственный комментарий может подтверждать только то, что участник употреблял мем или обсуждал тему.
