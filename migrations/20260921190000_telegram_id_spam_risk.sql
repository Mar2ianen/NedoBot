alter table telegram_new_user_profile_audits
    add column if not exists telegram_user_id_spam_probability double precision
        check (telegram_user_id_spam_probability >= 0 and telegram_user_id_spam_probability <= 1),
    add column if not exists telegram_user_id_risk_model_version text;

create index if not exists telegram_new_user_profile_audits_id_spam_probability_idx
    on telegram_new_user_profile_audits (chat_id, telegram_user_id_spam_probability desc)
    where telegram_user_id_spam_probability is not null;
...
select chat_id, telegram_user_id, analyzed_at, first_seen_at, last_seen_at,
       ...
       telegram_user_id_spam_probability, telegram_user_id_risk_model_version
