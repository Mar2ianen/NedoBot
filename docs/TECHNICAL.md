...
Для `/ask` финальная модель ответа сначала проходит LLM time formatter с явно захваченным `now`, затем compiled `RenderedMessage.rich_message` передаётся в Drafter без повторного рендера. Готовый Markdown валидируется до статуса `delivery_pending`; только после подтверждённой доставки run становится `completed`. При `NotAttempted` или подтверждённом `Rejected` допускается безопасный fallback, при `Unknown` второе сообщение запрещено. `ask_runs` сохраняет исходный Markdown отдельно от compiled Markdown, captured `now`, dialect, timezone, renderer revision и delivery outcome/certainty для аудита и immutable replay.
