...
- `chat.search_messages` по умолчанию использует `match_mode: "hybrid"`: русский/simple full-text плюс короткое fuzzy-сопоставление через `pg_trgm`. Допустимы `full_text`, `any_terms`, `literal` и `whole_word`; для альтернативных формулировок используй `any_terms`, для точного термина — `whole_word` или `literal`. Результат имеет форму `{messages, total_count, has_more, next_offset}`; для следующей страницы передай возвращённый `next_offset` как `offset` (допустимый диапазон 0–10000), поэтому top-k не следует принимать за полный набор.
- `chat.search_messages_batch` выполняет до шести независимых запросов и возвращает метаданные `total_count`/`has_more`/`next_offset` для каждого запроса. `chat.count_messages` возвращает точное количество строк с теми же фильтрами и предназначен для вопросов «сколько сообщений»/«сколько раз», а не для подсчёта уникальных авторов.
