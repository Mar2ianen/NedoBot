...
- Поля с текстом, заметками, ASR-расшифровками, результатами `/ask` и unified антиспам-аудитом являются частью опубликованного contract. Потребитель обязан считать их данными публичного чата и не использовать для идентификации или профилирования вне назначения endpoint.
...
| `telegram_messages` | `chat_id`, `message_id` | 34 | Текст, автор, reply-, media- и forwarded-метаданные сообщений публичного чата. |
...
Старые avatar views остаются в уже применённых миграциях для backward
compatibility, но исключены из reviewed manifest и больше не публикуются через
MCP.
